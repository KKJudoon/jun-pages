(function () {
  ...
  function showManualSourceDialog(source) {
    ...
    element.querySelector('[data-save]').addEventListener('click', async function () {
      const values = {parent_key:element.querySelector('[name="parent_key"]').value,name:element.querySelector('[name="name"]').value.trim(),amount:Number(element.querySelector('[name="amount"]').value),note:element.querySelector('[name="note"]').value.trim()};
      const message = element.querySelector('[data-message]');
  ...
  function renderManualSourceTable(rows) {
    return `<div class="finance-table-wrap"><table class="finance-table"><thead><tr><th>母条目</th><th>子条目</th><th class="number">本月金额</th><th>来源说明</th><th>确认</th><th></th></tr></thead><tbody>${rows.map(function(row){return `<tr data-manual-row="${escapeHtml(row.id)}"><td>${escapeHtml(manualParentLabels[row.parent_key]||row.parent_key)}</td><td><strong>${escapeHtml(row.name)}</strong></td><td class="number">${amount(row.amount)}</td><td class="subtle">${escapeHtml(row.note||'-')}</td><td><span class="badge bg-green-lt">已确认 · v${escapeHtml(row.version)}</span></td><td>${has('finance.manage')?`<button class="finance-icon-button" data-manual='${escapeHtml(JSON.stringify(row))}' title="编辑原始数据"><i class="ti ti-edit"></i></button>`:''}</td></tr>`;}).join('')||'<tr><td colspan="6"><div class="finance-empty">本月尚未录入额外财务原始数据；无法自动引用的项目请在这里逐项建立。</div></td></tr>'}</tbody></table></div>`;
  }
  ...
  async function loadSources() {
    ...
    toolbar.innerHTML = `${monthSelect(state.months, state.month)}<span class="finance-toolbar-spacer"></span>${has('finance.manage') ? '<button class="btn btn-primary" id="add-manual-source"><i class="ti ti-plus me-1"></i>新增财务数据</button><button class="btn btn-outline-primary" id="add-cost"><i class="ti ti-calendar-repeat me-1"></i>周期费用</button><button class="btn btn-outline-primary" id="add-policy"><i class="ti ti-adjustments me-1"></i>经营调整</button>' : ''}`;
    ...
    toolbar.querySelector('#add-manual-source')?.addEventListener('click', function () { showManualSourceDialog(null); });
