(function () {
  ...
  async function loadFinanceMonths() {
    ...
    const fallback = page === 'finance-employees' ? monthBeforeNow() : state.months[0]?.month || monthBeforeNow();
  ...
  function analysisDifference(current, previous, type, favorableDirection) {
    const currentValue = numeric(current);
    const previousValue = numeric(previous);
    if (currentValue == null || previousValue == null) return '<span class="analysis-delta unavailable">暂无可比数据</span>';
    const difference = currentValue - previousValue;
    const relative = previousValue === 0 ? null : difference / Math.abs(previousValue) * 100;
    const sign = difference > 0 ? '+' : difference < 0 ? '−' : '';
    const absolute = type === 'ratio'
      ? `${sign}${Math.abs(difference).toFixed(2)} 个百分点`
      : type === 'multiple'
        ? `${sign}${Math.abs(difference).toFixed(2)}×`
        : `${sign}${amount(Math.abs(difference))}`;
    const relativeText = relative == null ? '基期为 0' : `${relative > 0 ? '+' : relative < 0 ? '−' : ''}${Math.abs(relative).toFixed(1)}%`;
    const tone = difference === 0
      ? 'flat'
      : favorableDirection === 'neutral'
        ? 'change'
        : ((difference > 0) === (favorableDirection === 'up') ? 'good' : 'bad');
    return `<span class="analysis-delta ${tone}"><strong>差 ${absolute}</strong><small>同比 ${relativeText}</small></span>`;
  }

  function comparisonMetricCell(current, previous, field, type, favorableDirection) {
    const currentValue = current?.[field];
    const previousValue = previous?.[field];
    const comparable = current && previous && current.sales_complete && previous.sales_complete && currentValue != null && previousValue != null;
    const currentDisplay = current
      ? analysisValue(current, field, analysisDisplay(currentValue, type))
      : '<span class="analysis-value-missing">待有数据</span>';
    const previousDisplay = previous
      ? analysisValue(previous, field, analysisDisplay(previousValue, type))
      : '<span class="analysis-value-missing">待有数据</span>';
    const comparison = comparable
      ? analysisDifference(currentValue, previousValue, type, favorableDirection)
      : `<span class="analysis-delta unavailable">${current && !current.sales_complete ? '本月未完整，暂不同比' : '暂无可比数据'}</span>`;
  ...
  function renderYearComparisonTable(items) {
    ...
    const currentYear = years.at(-1);
    const previousYear = String(Number(currentYear) - 1);
    if (!currentYear || !years.includes(previousYear)) return '<div class="finance-analysis-empty">目前只有一个年份的可核验经营数据；第二年数据形成后会自动按同月份紧邻比较。</div>';
    const byMonth = new Map(items.map(function (item) { return [item.month, item]; }));
  ...
  function employeePayoutHistory(employee) {
    const history = (state.employeeFoundation?.payout_history || []).filter(function(h) { return h.employee_id === employee?.id; });
    if (!history.length) return '';
    return `<details class="finance-section"><summary>发放资料变更记录（${history.length}）</summary>${history.map(function(h) { return `<p>${escapeHtml(h.effective_month)} 工资起 · ${h.payment_kind === 'non_corporate' ? '非对公发放' : '对公发放'} · ${escapeHtml(h.payment_method)} · ${escapeHtml(h.payment_account)} · ${escapeHtml(h.payment_bank)} · ${escapeHtml(h.changed_by_name || '')} · ${escapeHtml(h.changed_at)}</p>`; }).join('')}</details>`;
  }

  function payoutKind(row) { return row.payload?.payment_kind === 'non_corporate' ? 'non_corporate' : 'corporate'; }
  function payoutGroupTitle(rows, kind) {
    const group = rows.filter(function(row) { return payoutKind(row) === kind; });
    const total = group.some(function(row) { return row.net_pay == null; }) ? null : group.reduce(function(sum,row) { return sum + Number(row.net_pay); },0);
  ...
  function renderPayroll() {
    ...
    content.innerHTML = `${reportTabs()}<div class="payroll-sheet">
      ...
      ${totalBand}
      <div class="payroll-payment-list">${rows.map(function (row, index) {
        ...
        const kind = payoutKind(row);
        const heading = index === 0 || payoutKind(rows[index - 1]) !== kind ? payoutGroupTitle(rows,kind) : '';
        return `${heading}<article class="payroll-payment-row ${statusClass} ${kind === 'non_corporate' ? 'is-non-corporate' : ''}">
          ...
          <div class="payroll-payment-bank"><i class="ti ${kind === 'non_corporate' ? 'ti-wallet' : 'ti-building-bank'}"></i><span>${escapeHtml([row.payment_method, row.payment_bank].filter(Boolean).join(' · ') || '付款资料待补')}</span><strong>${escapeHtml(row.payment_account || '账号待补')}</strong></div>
  ...
  function exportPayroll() {
    ...
    rows.forEach(function (row, index) {
      ...
      const insured = row.payload?.has_social_insurance === true;
      const selfFunded = insured && row.payload?.self_funded_social === true;
      const readyChecks = [`COUNT(F${excelRow})=1`, `COUNT(G${excelRow})=1`];
      if (insured) readyChecks.push(`COUNT(H${excelRow}:J${excelRow})>0`);
      if (selfFunded) readyChecks.push(`COUNT(K${excelRow})=1`);
      // SUM ignores empty text cells; direct subtraction produces #VALUE! in Excel/WPS.
      const netFormula = `IF(AND(${readyChecks.join(',')}),ROUND(F${excelRow}-SUM(G${excelRow}:${selfFunded ? 'K' : 'J'}${excelRow}),2),"")`;
      const cells = values[index + 1];
      const isNumber = function (value) { return typeof value === 'number' && Number.isFinite(value); };
      const ready = isNumber(cells[5]) && isNumber(cells[6]) && (!insured || cells.slice(7,10).some(isNumber)) && (!selfFunded || isNumber(cells[10]));
      const net = ready ? Math.round((cells[5] - cells.slice(6,selfFunded ? 11 : 10).reduce(function(sum,value) { return sum + (isNumber(value) ? value : 0); },0)) * 100) / 100 : '';
      sheet[`L${excelRow}`] = {t: ready ? 'n' : 's', f: netFormula, v: net, z: '#,##0.00'};
    ...
    for (let column = 6; column <= 12; column += 1) {
      ...
      const total = rows.reduce(function (sum, row) {
        ...
        if (letter === 'K') return sum + Number(row.payload?.has_social_insurance ? row.employer_social_insurance || 0 : 0);
        return sum + Number(sheet[`L${rows.indexOf(row) + 2}`].v || 0);
      }, 0);
      const pendingNet = letter === 'L' && rows.some(function (_,index) { return sheet[`L${index + 2}`].v === ''; });
      sheet[`${letter}${totalRow}`] = {t: pendingNet ? 's' : 'n', f: letter === 'L' ? `IF(COUNT(L2:L${totalRow - 1})=${rows.length},ROUND(SUM(L2:L${totalRow - 1}),2),"")` : `ROUND(SUM(${letter}2:${letter}${totalRow - 1}),2)`, v: pendingNet ? '' : Math.round(total * 100) / 100, z: '#,##0.00'};
    }
    sheet[`C${totalRow}`] = {t:'s',v:'合计'};
  ...
  function renderEmployees() {
    ...
    content.innerHTML = `${reportTabs()}<section class="employee-panel"><div class="employee-summary"><div><strong>${employees.filter(employeeActiveForMonth).length}</strong><span>本月在册</span></div><div><strong>${employees.filter(function (item) { return employeeActiveForMonth(item) && item.has_social_insurance; }).length}</strong><span>本月有医社保</span></div><div><strong>${employees.filter(function (item) { return employeeActiveForMonth(item) && item.included_in_company_report === false; }).length}</strong><span>工资不计经营财报</span></div></div><div class="finance-table-wrap"><table class="finance-table employee-table"><thead><tr><th>工号 / 姓名</th><th>部门 / 职位</th><th>计薪方式</th><th class="number">基础工资</th><th>医社保</th><th>需要财务核算</th><th>工资条账号</th><th>付款资料</th><th>状态</th><th></th></tr></thead><tbody>${employees.map(function (employee) {
      ...
      return `<tr class="${activeThisMonth ? '' : 'is-inactive'}"><td><strong>${escapeHtml(employee.employee_no)} · ${escapeHtml(employee.employee_name)}</strong><div class="finance-note">${escapeHtml(employee.nickname || '')}</div></td><td>${escapeHtml(employee.department)}<div class="finance-note">${escapeHtml(employee.role_name)}</div></td><td>${escapeHtml(employee.compensation_method || employeeRule(employee.id)?.rule_type || '-')}</td><td class="number">${amount(employee.base_salary)}</td><td>${employee.has_social_insurance ? '有' : '无'}</td><td>${employee.requires_finance_accounting === false ? '否' : '是'}</td><td>${profile ? `<strong>${escapeHtml(profile.display_name || profile.username)}</strong><div class="finance-note">${escapeHtml(profile.username)}</div>` : '<span class="finance-note">未绑定</span>'}</td><td>${escapeHtml(employee.payment_bank || '-')}<div class="finance-note">${escapeHtml(maskValue(employee.payment_account, 4, 4))}</div></td><td>${status}</td><td><button class="finance-icon-button" data-employee="${escapeHtml(employee.id)}" title="查看并编辑员工资料"><i class="ti ti-edit"></i></button></td></tr>`;
    }).join('') || '<tr><td colspan="10"><div class="finance-empty">员工主档尚未导入</div></td></tr>'}</tbody></table></div></section>`;
  ...
  function showEmployeeDialog(employee) {
    ...
    element.querySelector('[data-save]').addEventListener('click', async function () {
      ...
      values.requires_finance_accounting = element.querySelector('[name="requires_finance_accounting"]').checked;
      values.accounting_change_reason = value('accounting_change_reason');
      const payload = {month:state.month,id:employee?.id || null,expected_version:employee?.version || null,values,rule:{rule_type:value('rule_type'),start_month:value('rule_start_month'),end_month:rule?.end_month || '',parameters,note:value('note')}};
      const message = element.querySelector('[data-message]');
      try {
        ...
        if (saved.payout_change) {
          window.alert(`发放资料已保存，从 ${saved.payout_change.effective_month} 工资起生效${saved.payout_change.deferred ? '（已有工资发放，自动顺延）' : ''}`);
        }
        if (saved.accounting_change) {
          const change = saved.accounting_change;
          const date = new Date(`${change.effective_month}-01T12:00:00`); date.setMonth(date.getMonth()+1);
          const payout = `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}`;
          window.alert(`${change.deferred ? '该员工工资已发放，本次修改顺延。' : '设置已生效。'}从 ${change.effective_month} 工资起${change.requires_finance_accounting ? '需要' : '不需要'}财务核算（${payout} 发放）。`);
        }
  ...
  function showManualSourceDialog(source) {
    ...
    const requiredSource = source?.is_required === true;
    const element = dialog('finance-manual-source-dialog', source ? '编辑财务原始数据' : '新增财务原始数据', `<p class="finance-note">${requiredSource ? '这是每月必填项目；当月没有金额也要明确填 0 并写明依据。' : '金额只在这里维护；月报按所选母条目自动引用。'}保存即代表管理员已核对，并保留版本记录。</p><label>母条目<select class="form-select" name="parent_key" ${requiredSource ? 'disabled' : ''}>${options}</select></label><label>子条目名称<input class="form-control" name="name" maxlength="120" required ${requiredSource ? 'disabled' : ''} value="${escapeHtml(source?.name || '')}"></label><label>本月金额<input class="form-control" type="number" step="0.01" name="amount" required value="${escapeHtml(source?.amount ?? '')}"></label><label>来源与核对说明<textarea class="form-control" name="note" rows="3" required placeholder="例如：7月电费账单，财务回单核对">${escapeHtml(source?.note || '')}</textarea></label><div class="finance-error" data-message hidden></div><footer><button class="btn btn-outline-secondary" value="cancel">取消</button>${source && !requiredSource ? '<button class="btn btn-outline-danger me-auto" type="button" data-retire>停用本条</button>' : ''}<button class="btn btn-primary" type="button" data-save>确认并计入月报</button></footer>`);
  ...
  function renderManualSourceTable(rows) {
    return `<div class="finance-table-wrap"><table class="finance-table"><thead><tr><th>母条目</th><th>子条目</th><th class="number">本月金额</th><th>来源说明</th><th>确认</th><th></th></tr></thead><tbody>${rows.map(function(row){const approved=row.status==='approved';return `<tr data-manual-row="${escapeHtml(row.id)}"><td>${escapeHtml(manualParentLabels[row.parent_key]||row.parent_key)}</td><td><strong>${escapeHtml(row.name)}</strong>${row.is_required===true?'<span class="badge bg-red-lt ms-1">每月必填</span>':''}</td><td class="number">${amount(row.amount)}</td><td class="subtle">${escapeHtml(row.note||'待填写来源依据')}</td><td><span class="badge ${approved?'bg-green-lt':'bg-yellow-lt'}">${approved?'已确认':'待确认'} · v${escapeHtml(row.version)}</span></td><td>${has('finance.manage')?`<button class="finance-icon-button" data-manual='${escapeHtml(JSON.stringify(row))}' title="${approved?'编辑':'填写并确认'}原始数据"><i class="ti ti-edit"></i></button>`:''}</td></tr>`;}).join('')||'<tr><td colspan="6"><div class="finance-empty">本月尚未建立人工财务源槽位。</div></td></tr>'}</tbody></table></div>`;
  }
  ...
  async function init() {
    ...
    markNavigation();
    if (page === 'production-manual') { const script = document.createElement('script'); script.src = '/jun-pages/manual-approvals.js?v=20260909-batch2'; script.onerror = () => renderError(new Error('手工审批页面加载失败，请刷新重试')); document.head.appendChild(script); return; }
    if (page === 'production-pattern') { const script = document.createElement('script'); script.src = '/jun-pages/pattern-approvals.js?v=20260906-1'; script.onerror = () => renderError(new Error('制版审批页面加载失败，请刷新重试')); document.head.appendChild(script); return; }
