...
function render(){
 ...
 $('#module-toolbar').innerHTML=`<div class="show-toolbar"><label>结算月<input id="show-month" type="month" class="form-control" value="${esc(month)}" required></label>${data.can_submit?'<button type="button" class="btn btn-primary" data-new-show>＋ 申请走秀活动</button>':''}<button type="button" class="btn btn-outline-secondary" data-refresh-show>刷新</button></div>`;
...
function submitForm(d,makeValues){
 ...
 try{const values=makeValues(new FormData(e.target));await api(values);if(values.action==='submit'){month=values.values.month;const u=new URL(location);u.searchParams.set('month',month);history.replaceState(null,'',u);}done=true;d.close();try{await load();}catch(err){message('操作已成功，但列表刷新失败，请点击刷新核对。');}}
...
function newShow(){
 ...
 const employees=data.employees||[],own=employees.find(e=>e.id===data.employee_id);
 const recipient=data.can_choose_employee?`<label>工资归属人<select name="employee_id" class="form-control" required><option value="">请选择工资归属人</option>${employees.map(e=>`<option value="${esc(e.id)}">${esc(e.name)}</option>`).join('')}</select></label>`:`<p>工资归属人：<strong>${esc(own?.name||'账号尚未关联员工，请联系管理员')}</strong></p>`;
 const d=dialog('提交申请',`<label>结算月<input name="month" type="month" class="form-control" value="${esc(month)}" required></label>${recipient}<label>金额（元）<input name="amount" type="number" class="form-control" inputmode="decimal" min="0.01" max="99999999.99" step="0.01" required></label><label>备注<textarea name="note" class="form-control" rows="4" maxlength="2000" placeholder="例如：活动名称、合作内容" required></textarea></label><p class="text-secondary">审批通过后自动计入所选结算月的走秀合作工资。</p>`);
 submitForm(d,f=>{const values={month:String(f.get('month')),amount:String(f.get('amount')),note:String(f.get('note')).trim()};if(data.can_choose_employee)values.employee_id=String(f.get('employee_id'));return {action:'submit',values:{...values,id:requestId}};});
}
