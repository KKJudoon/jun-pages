(function () {
  ...
  function openPermissions(user) {
    ...
    document.getElementById('permission-groups').innerHTML = Object.entries(modules).map(function (entry) {
      return `<section class="permission-override-group"><h4>${escapeHtml(entry[0] === 'customer' ? '客服' : entry[0])}</h4>${entry[1].map(function (permission) {
        ...
        const adminOnly = permission.permission_key.startsWith('finance.');
