(function () {
  ...
  function openPermissions(user) {
    ...
    document.getElementById('permission-groups').innerHTML = Object.entries(modules).map(function (entry) {
      return `<section class="permission-override-group"><h4>${escapeHtml(entry[0] === 'customer' ? '客服' : entry[0] === 'business' ? '商务' : entry[0])}</h4>${entry[1].map(function (permission) {
        ...
        const adminOnly = (permission.permission_key.startsWith('finance.') || permission.permission_key === 'business.show.approve');
        return `<label class="permission-override-row"><div><strong>${escapeHtml(permission.label)}</strong><small>${escapeHtml(permission.description || '')}${adminOnly ? ' · 固定仅管理员' : ` · 角色默认${inherited ? '允许' : '拒绝'}`}</small></div><select data-permission="${permission.permission_key}" ${user.role === 'admin' || adminOnly ? 'disabled' : ''}><option value="inherit" ${adminOnly || current === 'inherit' ? 'selected' : ''}>${adminOnly ? '仅管理员' : '继承角色'}</option>${adminOnly ? '' : `<option value="true" ${current === 'true' ? 'selected' : ''}>明确允许</option><option value="false" ${current === 'false' ? 'selected' : ''}>明确拒绝</option>`}</select></label>`;
      }).join('')}</section>`;
    }).join('');
